...
{
    // ServiceLocator because singletons are not nice,
    // Considered DI but unity will handle what i need
    // Git amend MVP: https://github.com/adammyhre/Unity-Service-Locator/tree/master
    ...
    internal void VerifyGlobalServices() 
    {
        foreach(var type in RequiredServices.CoreServices) {
            if(!services.RegisteredTypes.Contains(type)) 
                throw new Exception($"ServiceLocator.Global: Missing required service of type {type.FullName}");
        }
    }
